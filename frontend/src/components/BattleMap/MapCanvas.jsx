...
export default function MapCanvas({ roomId, isGm, canMoveToken, canManageToken, onDropTemplate }) {
  ...
  const setTokenInInitiative = useBattleMapStore((s) => s.setTokenInInitiative);
  const setInitiativeStatsVisible = useBattleMapStore((s) => s.setInitiativeStatsVisible);
    ...
    >
      ...
      {showTokenPanel && (
        <TokenActionPanel
          ...
          inInitiative={!!selectedToken.in_initiative}
          initiativeStatsVisible={!!selectedToken.initiative_stats_visible_to_players}
          onToggleInitiative={() => setTokenInInitiative(roomId, selectedToken.id, !selectedToken.in_initiative)}
          onToggleInitiativeStatsVisible={() => setInitiativeStatsVisible(roomId, selectedToken.id, !selectedToken.initiative_stats_visible_to_players)}
          onDelete={() => {
            if (!window.confirm('Удалить токен с карты?')) return;
